...
import os
from data.auth_data import polina
...
def search_by_author():
    ...
    if form.validate_on_submit():
        ...
        if not author:
            return render_template('searchauthor.html', title='Фильтрация по автору', form=form,
                                   message='Книг такого автора у нас нет')
...
def contacts():
    """Страница "Контакты" """
    return render_template('contacts.html', title='Контакты', polina=polina)
